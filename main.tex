...
\begin{abstract}
  ...
  Incremental computations repeatedly evaluate a function on
  ...
  size of the changes of the (input and output) data, rather than to
  the size of the entire dataset.

  In the area of databases the incremental computation problem is
  known as Incremental View Maintenance (IVM); IVM has long been a
  central problem in database theory.  Many solutions have been
  proposed for restricted classes of computation, but we are
  seeking a general solution.

  As a first step of our solution, we define incremental computations
  as computations on data streams, i.e., sequences of data values.  We
  borrow ideas from the theory of Digital Signal Processing, which is
  a form of computation over streams.
  ...
  program, and (4) we show that practical database query languages,
  such as SQL and Datalog, can be directly implemented on top of
  \dbsp.  As a consequence, we obtain efficient incremental view
  maintenance algorithms for essentially arbitrary queries written in
  all these languages.
\end{abstract}
