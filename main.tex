...
\begin{abstract}
  ...
  directly implemented on top of \dbsp, using only primitive operators
  that can be efficiently incrementalized.  As a consequence, we
  obtain a general recipe for efficient IVM for essentially arbitrary
  queries written in all these languages.
\end{abstract}
