...
\begin{abstract}
  ...
  size of the changes of the, rather than to the size of the entire
  dataset.
  ...
  central problem in database theory and practice.  Many solutions
  have been proposed for restricted classes of computation, but we are
  seeking a general solution.

  We start by defining incremental computations as computations on
  data streams, i.e., sequences of data values.  We borrow ideas from
  ...
  program by reducing the problem of incrementalizing a complex query
  to the problem of incrementalizing the primitive operations that the
  query is built from, and (4) we show that practical database query
  languages, such as SQL and Datalog, can be directly implemented on
  top of \dbsp, using only primitive operators that can be efficiently
  incrementalized.  As a consequence, we obtain efficient incremental
