...
\begin{abstract}
  ...
  Incremental computations repeatedly evaluate a function $Q$ on a
  some input values that are ``changing''.  The goal of an efficient
  implementation is to ``reuse'' previously computed results as much
  as possible.  Ideally, when presented with a new change to the
  input, an incremental computation should only perform work
  proportional to the size of the changes of the (input and output)
  data, rather than to the size of the entire dataset.

  In the area of databases the canonical incremental computation
  problem is Incremental View Maintenance (IVM); IVM has long been a
  central problem in database theory.  Many solutions have been
  proposed for restricted classes of computations, but we are seeking
  a general solution.

  As a first step of our solution, we model incremental computations
  as computations on data streams, i.e., sequences of data values.  We
  borrow ideas from the theory of Digital Signal Processing, which has
  always been concerned with streaming computations.
  ...
  incremental computation; (3) we give a general algorithm for
  efficiently implementing incremental computations for arbitrary
  \dbsp programs, and (4) we show that practical database query
  languages, such as SQL and Datalog, can be directly implemented on
  top of \dbsp.  As a consequence, we obtain efficient incremental
  view maintenance algorithms for essentially arbitrary queries
