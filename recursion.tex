...
The \defined{delta function} (named from the Dirac delta function)
...
In this circuit $Q$ is a streaming operator, but the entire circuit
...
which evaluates the Datalog query; in \refsec{sec:nested} we derive
...
\item Lift this circuit to operate on streams:
    ...
    \end{tikzpicture}
    \end{center}
  Construct $\lift{R}$ by lifting each operator individually.
...
Think of the feedback cycle in this circuit as a \texttt{while} loop
that iterates until no changes are observed (in technical terms, a
fixed-point of $R$ is reached).  The $\D$ operator yields the set of
\emph{new changes} computed by each iteration of the loop.  When the
set of new changes becomes empty, the fixed point has been reached.
...
Note that this loop is not guaranteed to terminate.  But if a program
