...
In this circuit $S$ is a streaming operator, but the entire circuit
...
that iterates until no changes are observed (i.e., a fixed-point of
$R$ is reached).  The $\D$ operator yields the set of \emph{new
changes} computed by each iteration of the loop.  When the set of new
changes becomes empty, the fixed point has been reached.
