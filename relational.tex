...
tells us \emph{how many times} the row belongs to the table.
...
&
\parbox[b][][t]{\commentsize}{
  Project each value with its weight unchanged.
  Add up weights of identical values.
}
\\ \hline
Filtering &
\begin{lstlisting}[language=SQL]
...
&
\parbox[b][][t]{\commentsize}{
  Select each value separately.  If the value is selected preserve the
  weigth, else make the weight 0.
}
% \\ \hline
%Selection &
%\begin{lstlisting}[language=SQL]
...
&
\parbox[b][][t]{\commentsize}{
  Multiply the weights of the values.
}
\\ \hline
Equi-join &
\begin{lstlisting}[language=SQL]
...
&
\parbox[b][][t]{\commentsize}{
  Multiply the weights of the values that appear in the output.
}
\\ \hline
Intersection &
\begin{lstlisting}[language=SQL]
...
output change $\Delta o$.  The implementation needs to maintain the
\emph{entire input set} in order to discover whether an item is new or
not.  That is the purpose of the $\I$ operator in the above diagram.
The $i$ arrow is the previous version of the full input table.  The
function $H$ detects whether the weight of a row in $i$ is changing
sign (positive to negative or vice-versa) when the row appears in
$\Delta d$.  A precise definition of $H$ is given in the full paper.
