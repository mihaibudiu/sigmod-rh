...
\begin{tikzpicture}[node distance=1.1cm,>=latex]
    ...
    \node[block, right of=mid, node distance=.5cm] (j) {$\bowtie_{id = id}$};
    \node[block, right of=j, node distance=1.3cm] (p) {$\pi_{x, y}$};
    \node[block, right of=p] (d) {$\distinct$};
    \node[right of=d, node distance=.8cm] (V) {\code{V}};
    \draw[->] (t1) -- (s1);
    \draw[->] (s1) -- (d1);
    \draw[->] (d1) -- (p1);
    \draw[->] (p1) -- (d11);
...
Note that some arrows that were sets in the original circuit
may by multisets in the optimized circuit.
