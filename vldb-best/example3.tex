...
Here we take the \dbsp circuit for the transitive closure of a graph
...
\begin{tikzpicture}[>=latex, node distance=1.6cm]
  ...
  % relational query
  \node[block, above of=E, node distance=1cm] (j) {$\lift{\inc{(\lift{\bowtie})}}$};
  \node[block, right of=j, node distance=1.4cm] (pj) {$\lift{\lift{\pi}}$};
  \node[block, circle, below of=pj, node distance=1cm, inner sep=0cm] (plus) {$+$};
  \node[block, right of=plus, node distance=1.3cm] (d) {$\lift{\inc{(\lift{\distinct})}}$};
  \draw[->>>] (E) -- (j);
  \draw[->>>] (j) -- (pj);
  \draw[->>>] (E) -- (plus);
  \draw[->>>] (pj) -- (plus);
...
\begin{tikzpicture}[>=latex, node distance=1.6cm]
  ...
  % generic part
  \node[block, right of=Einput, node distance=1cm] (E) {$\inc{(\lift{\delta})}$};

  % relational query
  \node[block, above of=E, node distance=1cm] (j) {$\inc{(\lift{\inc{(\lift{\bowtie})}})}$};
  \node[block, right of=j, node distance=1.9cm] (pj) {$\inc{(\lift{\lift{\pi}})}$};
  \node[block, circle, below of=pj, node distance=1cm, inner sep=0cm] (plus) {$+$};
...
We now take advantage of the linearity of $\lift\delta$, $\lift\int$,
