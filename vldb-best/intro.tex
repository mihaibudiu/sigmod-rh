...
This paper is an extended version of~\cite{budiu-vldb23}, adopting the
notations from~\cite{budiu-sigmod24}.  The major additions are a
couple of examples:
(\refsec{sec:recursive-example},\refsec{sec:recursive-incremental-example}),
an expanded implementation section~\refsec{sec:implementation}, and an
experimental evaluation~\refsec{sec:experiments}.  This paper omits
most of the proofs to the theorems; the proofs can be found in an
extended technical report~\cite{tr}, and have also been formalized and
verified in the Lean proof assistant~\cite{dbsp-theory}.
...
stated as follows: we are given a large database $DB$ and a view $V$,
...
In this example the query $Q$ defining the view $V$ is \texttt{SELECT
  * FROM T WHERE Age >= 10}.  The view \code{V} always contains all
the rows of table \code{T} whose value for the column \code{Age} is
...
smaller than the database itself.  The ``up'' arrows show the
...
be an improvement of several orders of magnitude.
...
produces streams.  In general we use ``operator'' for streams, and
...
\begin{enumerate}
  ...
  \item The first general and machine-checked theory of IVM.  All the
    theoretical results in this paper~\cite{budiu-vldb23} have been
  ...
  \item An evaluation of the performance of our implementation.
\end{enumerate}
